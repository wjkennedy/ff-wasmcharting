...
export var getSerializedStylesMap = function getSerializedStylesMap(cssProperty, tokenMap) {
  return Object.keys(tokenMap).reduce(function (emotionMap, token) {
    emotionMap[token] = css(_defineProperty({}, cssProperty, tokenMap[token]));
    return emotionMap;
  }, {});
};
 ...
 */
var _resolveTokens = function resolveTokens(styleObj) {
  ...
  for (var _i = 0, _Object$entries = Object.entries(styleObj); _i < _Object$entries.length; _i++) {
    ...
    if (typeof value === 'string') {
      ...
      var looksLikeToken = value.includes('.') && /^[a-z]/.test(value);
      if (looksLikeToken) {
        // Try to resolve as a token
        var tokenMapForProp = tokensMap[key];
      ...
      resolved[key] = _resolveTokens(value);
    } else {
 ...
 */
export { _resolveTokens as resolveTokens };
export var media = {
  above: {
    xxs: '@media all',
