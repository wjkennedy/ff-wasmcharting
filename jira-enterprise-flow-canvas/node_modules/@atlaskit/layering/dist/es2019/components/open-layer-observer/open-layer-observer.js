...
function createInternalAPI() {
   ...
   */
  const namespaceToChangeListenerRegistry = new Map();
   ...
   */
  const namespaceToLayerRegistry = new Map();
  ...
  function callChangeListeners({
    ...
    if (namespace) {
      ...
      // iterating over it.
      Array.from(listenersForNamespace !== null && listenersForNamespace !== void 0 ? listenersForNamespace : []).forEach(listener => listener({
        count: newCount
      }));
    }
    ...
    Array.from(noNamespaceListeners).forEach(listener => listener({
      count: totalCount
    }));
  }
  ...
  function getCount({
    ...
    if (type) {
      ...
      return Array.from(namespaceToLayerRegistry.values()).reduce((acc, layers) => acc + getLayerCountWithFilter({
        layers,
        type
      }), 0);
    }
  ...
  function onChange(listener, {
    ...
    const namespace = providedNamespace !== null && providedNamespace !== void 0 ? providedNamespace : noNamespaceSymbol;
 ...
 */
export function OpenLayerObserver({
  children
}) {
