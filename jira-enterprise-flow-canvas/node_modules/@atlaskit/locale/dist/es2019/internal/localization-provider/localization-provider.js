...
export const createLocalizationProvider = (locale, formatterOptions) => {
  ...
  const formatDate = date => Intl.DateTimeFormat(normalizedLocale).format(date);
  const formatTime = date => Intl.DateTimeFormat(normalizedLocale, {
    hour: 'numeric',
    minute: 'numeric'
  }).format(date);
  const getDays = (weekStartDay = 0, length) => {
    ...
    const rotatedWeekdays = weekStartDay > 0 ? [...weekdays.slice(weekStartDay), ...weekdays.slice(0, weekStartDay)] : weekdays;
  ...
  const getDaysShort = (weekStartDay = 0) => {
    return getDays(weekStartDay, 'short');
  };
  const getDaysLong = (weekStartDay = 0) => {
  ...
  const parseDate = (date, options = {}) => createDateParser(normalizedLocale)(date, options);
  const formatToParts = (date = new Date()) => {
