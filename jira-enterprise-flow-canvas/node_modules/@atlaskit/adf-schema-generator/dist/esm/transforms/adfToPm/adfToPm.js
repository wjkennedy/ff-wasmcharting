...
export function transform(adf) {
  ...
  var markResMap = {};
  var nodeGroupMap = {};
  var ignoredGroups = new Set(Array.from(PSEUDO_GROUPS));
  traverse(adf, {
      ...
      function processNode(node) {
        ...
        if (!cycle) {
          ...
          nodeResMap[nodeName] = {
            ...
            nodeTypeDefinition: buildNodeTypeDefinition(nodeSpec, node.getType(), (_nodeSpec$marks$map = (_nodeSpec$marks = nodeSpec.marks) === null || _nodeSpec$marks === void 0 ? void 0 : _nodeSpec$marks.map(function (m) {
              return m.getType();
    ...
    group: function group(_group, members) {
      if (_group.isIgnored(PMSpecTransformerName)) {
      ...
      nodeGroupMap[_group.group] = _group.members.filter(function (node) {
        ...
        return getNodeNames(node);
      });
      return {
        group: _group.group,
        members: members.filter(function (m) {
          return m && !m.node.isIgnored(PMSpecTransformerName);
        })
      };
    },
      ...
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          ...
          if (isGroupReturnValue(child)) {
            ...
            if (PSEUDO_GROUPS.has(child.group)) {
              ...
              try {
                for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                  ...
                  contentTypes.push.apply(contentTypes, _toConsumableArray(getNodeNames(member.node)));
                }
              } catch (err) {
                _iterator2.e(err);
              } finally {
            ...
            contentTypes.push.apply(contentTypes, _toConsumableArray(getNodeNames(child.node)));
    ...
    $range: function $range(item, child) {
      return {
        // PM Spec does not have concept of variants, so we will have duplicate content types
        expr: [buildContentExpression(Array.from(new Set(child.expr)), "{".concat(item.min, ",").concat(item.max, "}"))],
        marks: child.marks,
        contentTypes: child.contentTypes
      };
    }
