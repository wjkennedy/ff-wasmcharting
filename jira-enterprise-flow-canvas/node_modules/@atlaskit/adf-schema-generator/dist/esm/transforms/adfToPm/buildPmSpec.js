...
export var buildAttrs = function buildAttrs(attrs) {
  ...
  if (attrs['anyOf']) {
    attributes = merge.apply(void 0, [{}].concat(_toConsumableArray(attrs['anyOf'])));
  }
  // @ts-expect-error
  return Object.entries(attributes).reduce(function (acc, _ref) {
    ...
    return _objectSpread(_objectSpread({}, acc), {}, _defineProperty({}, key, _objectSpread(_objectSpread({}, acc[key]), 'default' in value && {
...
export var buildNodeSpec = function buildNodeSpec(nodeType, nodeSpec, nodeGroups, content) {
  ...
  if (nodeType in NODES_MARKS_OVERRIDES) {
    ...
    marks = Array.apply(void 0, _toConsumableArray(new Set(marks))).filter(filterMarks(nodeType));
  }
  if (content.length) {
