...
node, content) {
  var isTupleLike = content.length > 1;
  if (isTupleLike) {
...
function buildTupleLikeContent(node, content) {
  var nodeSpec = node.getSpec();
  var hasUnsupportedInline = content.some(function (item) {
    return item.hasUnsupportedInline;
  });
  var hasUnsupportedBlock = content.some(function (item) {
    return item.hasUnsupportedBlock;
  });
  var returnContent = {
    type: 'array',
    isTupleLike: true
