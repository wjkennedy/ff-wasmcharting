...
exports.determineItems = determineItems;
exports.determineMaxItems = determineMaxItems;
exports.determineMinItems = determineMinItems;
exports.flattenArray = flattenArray;
exports.flattenContent = flattenContent;
exports.isADFGroup = isADFGroup;
exports.isADFNode = isADFNode;
exports.processContentGroups = processContentGroups;
...
var _nameResolver = require("./nameResolver");
...
function buildContent(content, name, nodeSpec, fullSchema) {
...
function determineItems(content, name, adfNodeContent, fullSchema) {
  ...
  if (name === 'doc') {
    var jsonSchema = adfNodeContent.reduce(function (acc, value) {
      ...
      if (value.type === '$one+' && value.content.type === '$or') {
        return [].concat((0, _toConsumableArray2.default)(acc), (0, _toConsumableArray2.default)(flattenContent(value.content.content).filter(function (node) {
          return !node.isIgnored(_transformerNames.JSONSchemaTransformerName) && !(node.isStage0Only() && fullSchema);
        }).map(function (node) {
          return {
            $ref: "#/definitions/".concat((0, _nameResolver.resolveName)(node.getName()))
          };
        })));
      }
      return acc;
    }, []);
    var anyOfItems = (0, _uniqBy.default)(jsonSchema, function (v) {
      return v.$ref;
    });
...
function processContentTypes(contentTypes) {
  ...
  contentTypes.forEach(function (piece) {
    itemsArray.push({
      $ref: "#/definitions/".concat((0, _nameResolver.resolveName)(piece))
    });
  });
...
function determineMinItems(content) {
...
function determineMaxItems(content) {
  // Despite it being possible for there to be multiple content groups on one node in DSL,
