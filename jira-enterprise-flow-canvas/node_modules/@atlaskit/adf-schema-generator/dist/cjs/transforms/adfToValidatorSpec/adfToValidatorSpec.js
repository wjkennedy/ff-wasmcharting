...
exports.adfToValidatorSpec = adfToValidatorSpec;
...
var _buildAttrs = require("./buildAttrs");
var _buildContent = require("./buildContent");
...
function transform(adf) {
  var result = {};
  (0, _traverse.traverse)(adf, buildVisitor(result, buildNode));
  var formattedResult = {};
...
function adfToValidatorSpec(adf) {
  return transform(adf);
}
var buildNode = exports.buildNode = function buildNode(node, content) {
  ...
  if (nodeSpec.attrs !== baseSpec.attrs && nodeSpec.attrs && Object.keys(nodeSpec.attrs).length > 0) {
    json.props.attrs = (0, _buildAttrs.buildAttrs)(nodeSpec.attrs);
  ...
  if (content.length && nodeSpec.content !== baseSpec.content) {
    json.props.content = (0, _buildContent.buildContent)(node, content);
  }
  if (nodeSpec.marks !== baseSpec.marks) {
    var _nodeSpec$marks;
    var marks = (0, _buildMarks.buildMarks)((_nodeSpec$marks = nodeSpec.marks) !== null && _nodeSpec$marks !== void 0 ? _nodeSpec$marks : [], nodeSpec.noMarks, nodeSpec.hasEmptyMarks);
    if (marks) {
...
var buildMarkValidatorSpec = function buildMarkValidatorSpec(mark) {
  ...
  if (markSpec !== null && markSpec !== void 0 && markSpec.attrs && Object.keys(markSpec.attrs).length > 0) {
    json.props.attrs = (0, _buildAttrs.buildAttrs)(markSpec.attrs);
  }
  return json;
};
var buildVisitor = function buildVisitor(res, buildNode) {
  return {
    node: function node(_node, content, cycle) {
      ...
      if (!_node.isIgnored(_transformerNames.ValidatorSpecTransformerName)) {
        res[_node.getName()] = processedNode;
      }
      return processedNode;
    },
    group: function group(_group, members) {
      if (_group.isIgnored(_transformerNames.ValidatorSpecTransformerName)) {
        return undefined;
      }
      var groupName = _group.group;
      res[groupName] = {
        json: _group.members.filter(function (member) {
          return !member.isIgnored(_transformerNames.ValidatorSpecTransformerName);
        }).map(function (member) {
    ...
    $or: function $or(children) {
      ...
      var hasNodes = children.some(function (item) {
        if (isNodeReturnValue(item)) {
          return !item.node.isIgnored(_transformerNames.ValidatorSpecTransformerName);
        }
        return false;
      });
          ...
          if (isGroupReturnValue(child)) {
              ...
              for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
                ...
                try {
                  for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
                    ...
                    if (!mark.isIgnored(_transformerNames.ValidatorSpecTransformerName)) {
            ...
            try {
              for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
                ...
                if (!_mark.isIgnored(_transformerNames.ValidatorSpecTransformerName)) {
